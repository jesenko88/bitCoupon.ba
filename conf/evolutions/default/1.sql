...
alter table report add constraint fk_report_comment_8 foreign key (comment_id) references comment (id) on delete restrict on update restrict;
create index ix_report_comment_8 on report (comment_id);
alter table report add constraint fk_report_user_9 foreign key (user_id) references user (id) on delete restrict on update restrict;
create index ix_report_user_9 on report (user_id);
alter table statistic add constraint fk_statistic_coupon_10 foreign key (coupon_id) references coupon (id) on delete restrict on update restrict;
create index ix_statistic_coupon_10 on statistic (coupon_id);
